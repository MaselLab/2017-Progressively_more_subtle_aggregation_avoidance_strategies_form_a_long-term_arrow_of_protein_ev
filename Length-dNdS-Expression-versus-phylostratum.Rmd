...
```{r}
APR.data <- APR.data[,c("EnsemblGeneUID","GeneFamilyPhylostratum","GeneFamilyNumber","PaxDBExpression","GeneOrControlDesignation","EvolutionaryRateHighestOrthologRate","ExcludedGeneBinary", "ProteinSeqLength")] #subset needed columns
...
```{r}
library(nlme)
fit.length.genes <- lme(bcLength ~ GeneFamilyPhylostratum , random = ~ 1|GeneFamilyNumber, data = APR.data[which(APR.data$GeneOrControlDesignation == "CodingGene"),]) #generate a linear mixed-effects model fit to mouse genes with gene family as random error
summary(fit.length.genes) #print fit AIC for mouse genes with gene family random error

fit.rate.genes <- lme(bcRate ~ GeneFamilyPhylostratum, random = ~ 1|GeneFamilyNumber, data = APR.data[APR.data$GeneOrControlDesignation == "CodingGene",]) #generate a linear mixed-effects model fit to mouse genes with gene family as random error, add dNdS as fixed term, exclude genes without a corresponding dNdS value for mouse-rat comparisons
summary(fit.rate.genes) #print fit AIC for evolutionary rate corrected linear model

fit.expression.genes <- lme(bcExpression ~ GeneFamilyPhylostratum, random = ~ 1|GeneFamilyNumber, data = APR.data[APR.data$GeneOrControlDesignation == "CodingGene",], na.action = "na.exclude") #generate a linear mixed-effects model fit to mouse genes with gene family as random error, add dNdS as fixed term, exclude genes without a corresponding dNdS value for mouse-rat comparisons
