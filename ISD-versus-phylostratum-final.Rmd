...
ISD.data$GeneFamilyPhylostratum <- with(ISD.data, factor(GeneFamilyPhylostratum, levels = c(sort(unique(ISD.data$GeneFamilyPhylostratum)),"Intergenic","Repeat-masked"))) # set levels of categorical phylostratum variable
ISD.data$Phylostratum <- as.factor(ISD.data$Phylostratum) #factorize variable to ensure that it is not interpreted as numeric variable
ISD.data$Phylostratum <- with(ISD.data, factor(Phylostratum, levels = c(sort(unique(ISD.data$Phylostratum)),"Intergenic","Repeat-masked"))) # set levels of categorical phylostratum variable
ISD.data$ExcludedGeneBinary <- as.factor(ISD.data$ExcludedGeneBinary) #factorize binary category for gene families with ambiguous age
```

Trim data frames to only the information needed
```{r}
ISD.data <- ISD.data[,c("EnsemblGeneUID","EnsemblProteinUID","Phylostratum","GeneFamilyPhylostratum","GeneFamilyNumber","NoCysIUPredMeanISD","GeneOrControlDesignation","ExcludedGeneBinary","EvolutionaryRateHighestOrthologRate")] #subset needed columns
...
ISD.data[which(ISD.data$GeneOrControlDesignation == "MaskedIntergenicNucleotideSeqControl"),]$GeneFamilyPhylostratum <- "Repeat-masked" #Give repeat-masked intergenic sequence a categorical phylostratum
ISD.data[which(ISD.data$GeneOrControlDesignation == "IntergenicNucleotideSeqControl"),]$Phylostratum <- "Intergenic" #Give intergenic sequence a categorical phylostratum for comparison
ISD.data[which(ISD.data$GeneOrControlDesignation == "MaskedIntergenicNucleotideSeqControl"),]$Phylostratum <- "Repeat-masked" #Give repeat-masked intergenic sequence a categorical phylostratum
```

Remove phylostratum 0 (unclassifiable), gene families with ambiguous age, genes without rat homologs, and ORFan proteins
```{r}
ISD.data <- ISD.data[which(ISD.data$GeneFamilyPhylostratum != "0" & ISD.data$GeneFamilyPhylostratum != "20" & ISD.data$ExcludedGeneBinary != "1" ),] #exclude unclassifiable genes, false-positive rich ORFans
```
...
plot.ISD <- ggplot(combined.df[which(combined.df$Model == "Genes"  |combined.df$Model == "Intergenic"|combined.df$Model =="Repeat-masked"),], aes(colour = Model, fill = Model, shape = Model))
plot.ISD <- plot.ISD + 
  geom_pointrange(aes(x = axis.breaks, y = bc.reverse.transform(Intercept, lambda1, lambda2), ymin = bc.reverse.transform(Intercept - SE, lambda1, lambda2), ymax = bc.reverse.transform(Intercept + SE, lambda1, lambda2)), size = 0.5, lwd = 0.5,   show.legend=FALSE) +
  
  
  annotate("text", x = axis.breaks[c(1:5,11:19)], y = rep(0.07,14), label = split.times[c(1:5,11:19)], angle=60, hjust = 0, vjust = 0.5, size =2.25) +
  annotate("text", x = -7.75, y = 0.1, label = "Avg. TMRCAs \n(mya)", size = 2.25 ) +
  ...
  scale_colour_manual(name = "", values = c(colBlack,colDarkBlue,colSkyBlue)) +
  scale_fill_manual(name = "", values = c(colBlack,colDarkBlue,colSkyBlue)) +
...
pdf(file = "ISDvsPSGenesIntergenicMasked.pdf", width =6.7, height = 3)
plot.ISD
dev.off()
